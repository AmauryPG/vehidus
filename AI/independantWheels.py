...
class Front_Wheels(object):
	...
	def turn(self, angle,bw):
		...
		ratio = getTurningRadiusRatio(angle)
		if ratio < 1.0:
			Back_Wheels.speed_left(Back_Wheels.speed_left(bw)*ratio)
		if ratio > 1.0:
			Back_Wheels.speed_rigth(Back_Wheels.speed_rigth(bw)*1-(ratio-1))
...
def getTurningRadiusRatio(angle):
	...
	if angle < 45*0.01745329 or  angle > 135*0.01745329 or angle == 90*0.01745329:
		return 0
	else:
...
if __name__ == '__main__':
	for angle in [10,20,30,40,45,55,65,75,85,90,95,105,115,125,135,145,155,165]:
